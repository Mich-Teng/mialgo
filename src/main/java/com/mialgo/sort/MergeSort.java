...
public class MergeSort  {
    ...
    static void merge(Comparable[] arr, int left, int pivot, int right, short mode){
        // index of leftside
        int l = left;
        // index of rightside
        int r = pivot+1;
        ...
        while( count <= right - left + 1 ){
            ...
            if( (mode == SortMode.ASCENDING)?((Comparable)arr[l]).compareTo(arr[r])<0:((Comparable)arr[l]).compareTo(arr[r])>0 )
                tmp[count++] = arr[l++];
            else
                tmp[count++] = arr[r++];
        }
